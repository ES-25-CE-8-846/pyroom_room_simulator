...
def amplitude_matching(ir_bright, ir_dark, ir_length, lambd):
    """
    Amplitude matching filter design with least-squares minimization.
    ...
    for s in range(S): # Compute filter for each source
        ...
        # Create target amplitude response
        d = np.zeros(H_b.shape[0])
        # Distribute the amplitude evenly across the time window for bright zone
        window_size = 64
        center = len(d) // 2
        window_start = center - window_size // 2
        window_end = center + window_size // 2

        hann_win = np.hanning(window_size)
        hann_win = hann_win / np.sqrt(np.sum(hann_win**2))  # Normalize energy

        d[window_start:window_end] = hann_win

        
        # Dark zone weight normalized by mic count
        dark_zone_weight = num_mics_bright / num_mics_dark
        
        # Amplitude matching - sqrt of the magnitude of the signals
        # Take absolute values of H matrices for amplitude matching
        H_b_amp = np.abs(H_b)
        H_d_amp = np.abs(H_d)
        
        # Cost function matrix for amplitude matching
        A = H_b_amp.T @ H_b_amp + lambd * dark_zone_weight * (H_d_amp.T @ H_d_amp) # Cost function matrix (first term is the bright zone error, second term is the dark zone energy)
        b = H_b_amp.T @ d  # Target amplitude in bright zone
...
def compare_signals(signal1, name1: str , signal2, name2: str):
    ...
    plt.plot(signal1, label=name1, alpha=0.9)
    plt.plot(signal2, label=name2, alpha=0.9)
...
def main():
    ...
    # Apply Amplitude Matching filtering
    print("Designing filters...")
    filters = amplitude_matching(ir_bright, ir_dark, ir_length, lambd=1e-2)  # Increase lambda for more dark zone influence
    ...
    plt.title("Amplitude Matching Filters for Each Speaker")
    ...
    AM_mic_signals = room.mic_array.signals  # shape: [num_mics, signal_len]
    AM_mic_signals = AM_mic_signals / global_norm  # Normalize

    AM_bright_mic_signal = AM_mic_signals[0]  # First bright mic
    AM_dark_mic_signal = AM_mic_signals[num_mics_bright]  # First dark mic

    # Compare the filtered signals from bright and dark zone
    compare_signals(AM_bright_mic_signal, "AM BZ", AM_dark_mic_signal, "AM DZ")

    # Compare filtered bright with original bright
    compare_signals(original_bright_mic_signal, "Original BZ", AM_bright_mic_signal, "AM BZ")

    # Compare filtered dark with original dark
    compare_signals(original_dark_mic_signal, "Original DZ", AM_dark_mic_signal, "AM DZ")

    sf.write("bright_mic_AM.wav", AM_bright_mic_signal, fs)
    sf.write("dark_mic_AM.wav", AM_dark_mic_signal, fs)
